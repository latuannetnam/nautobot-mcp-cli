...
| M3 | One bulk `cms_list(juniper_interface_families, ...)` call fetches all families for the device | L692–698: `all_families_resp = cms_list(client, "juniper_interface_families", InterfaceFamilySummary, device=device_id, limit=0)` | ✅ |
| M4 | Families are indexed by `unit_id` into a lookup dict (`unit_families`) | L699–701: `unit_families: dict[str, list[InterfaceFamilySummary]]` built via `setdefault(fam.unit_id, [])` | ✅ |
| M5 | One bulk `cms_list(juniper_interface_vrrp_groups, ...)` call fetches all VRRP groups for the device | L707–713: `all_vrrp_resp = cms_list(client, "juniper_interface_vrrp_groups", VRRPGroupSummary, device=device_id, limit=0)` | ✅ |
| M6 | VRRP groups are indexed by `family_id` into a lookup dict (`vrrp_by_family`) | L714–716: `vrrp_by_family: dict[str, list[VRRPGroupSummary]]` built via `setdefault(vg.family_id, [])` | ✅ |
