...
requirements: [BRG-01, BRG-02, BRG-03, BRG-04, BRG-05, BRG-06, BRG-07]
autonomous: true
---

# Plan 01: REST Bridge Module

<objective>
Build the `call_nautobot` REST bridge that dispatches CRUD operations to the correct backend (pynautobot core or CMS helpers) based on endpoint prefix, with endpoint validation, auto-pagination, device name resolution, fuzzy "did you mean?" suggestions, and structured error responses.
</objective>

<must_haves>
- Endpoint routing: `/api/*` → pynautobot core accessor, `cms:*` → CMS plugin
- Endpoint validation against catalog with `difflib.get_close_matches()` suggestions
- Auto-pagination for GET up to `limit` with hard cap at 200
- Device name → UUID auto-resolution for CMS endpoints
- HTTP error translation with structured hints (endpoint, method, hint)
- Wrapped response format: `{count, results, endpoint, method}` + truncation metadata
- `id` parameter support for single-object operations (GET by UUID, PATCH, DELETE)
</must_haves>

## Tasks

### Task 1: Create bridge module with endpoint validation and routing

<read_first>
- nautobot_mcp/catalog/engine.py (get_catalog() — used for endpoint validation)
- nautobot_mcp/catalog/core_endpoints.py (CORE_ENDPOINTS — endpoint path → app.endpoint mapping)
- nautobot_mcp/catalog/cms_discovery.py (CMS endpoint format: "cms:juniper_static_routes")
- nautobot_mcp/client.py (NautobotClient — api property, _handle_api_error(), dcim/ipam/etc. properties)
- nautobot_mcp/cms/client.py (CMS_ENDPOINTS registry, resolve_device_id(), get_cms_endpoint(), cms_list/get/create/update/delete)
- nautobot_mcp/exceptions.py (NautobotMCPError hierarchy, to_dict() pattern)
- docs/plans/2026-03-24-api-bridge-mcp-design.md §Tool 2: call_nautobot (routing logic, key behaviors)
- .planning/phases/16-rest-bridge-universal-crud/16-CONTEXT.md (all implementation decisions)
</read_first>

<action>
Create `nautobot_mcp/bridge.py` with a `call_nautobot()` function implementing the universal REST bridge.
...
The module is self-contained — it imports from `catalog` (for validation) and `cms.client` (for CMS dispatch) but introduces no new dependencies.
</action>

<acceptance_criteria>
- `nautobot_mcp/bridge.py` exists and contains `def call_nautobot(`
- `bridge.py` contains `MAX_LIMIT = 200`
- `bridge.py` contains `DEFAULT_LIMIT = 50`
- `bridge.py` contains `import difflib`
- `bridge.py` contains `def _validate_endpoint(`
- `bridge.py` contains `def _suggest_endpoint(`
- `bridge.py` contains `def _parse_core_endpoint(`
- `bridge.py` contains `def _execute_core(`
- `bridge.py` contains `def _execute_cms(`
- `bridge.py` contains `resolve_device_id` import from `nautobot_mcp.cms.client`
- `bridge.py` contains `get_catalog` import from `nautobot_mcp.catalog.engine`
- `from nautobot_mcp.bridge import call_nautobot` does not raise ImportError
- `_validate_endpoint("/api/dcim/devices/")` does not raise
- `_validate_endpoint("cms:juniper_static_routes")` does not raise
- `_validate_endpoint("/api/dcim/invalid/")` raises `NautobotValidationError` with "Did you mean"
- `_parse_core_endpoint("/api/dcim/devices/")` returns `("dcim", "devices")`
- `_parse_core_endpoint("/api/dcim/device-types/")` returns `("dcim", "device_types")`
</acceptance_criteria>
...
python -c "
from nautobot_mcp.bridge import _validate_endpoint, _suggest_endpoint, _parse_core_endpoint

# Valid endpoints pass
_validate_endpoint('/api/dcim/devices/')
_validate_endpoint('cms:juniper_static_routes')
print('Valid endpoints pass')
...
print('Core endpoint parsing OK')

# Invalid endpoint suggests alternatives
try:
    _validate_endpoint('/api/dcim/device/')
    assert False, 'Should have raised'
except Exception as e:
    assert 'Did you mean' in str(e) or 'Unknown endpoint' in str(e)
    print(f'Invalid endpoint error: {e}')

print('All bridge validation checks passed')
"
```

---
*Plan: 16-01 — REST Bridge Module*
